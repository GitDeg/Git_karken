...
repertoireMVC =    [pathname,sujet{suj},'\MVC']; % Changer pour le nom du r�pertoire avec tes essais au piano
...
for i=1%:length(c3dfilesMVC)
    ...
    for h=1:size(EMG_Vicon_Name,2)
        ...
        if im==1
            ...
            MVC(i).Labels    = [MVC(i).Labels EMG_Vicon_Name(h)] ; %Nom des entr�es EMG
        end
    end
    MVC(i).Rate = c3dDataMVC.parameters.ANALOG.RATE; %fr�quence d'�chantillonage.
