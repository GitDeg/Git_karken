...
for suj = 1 : 12
    ...
    for grpi = 1 : length(CINE_SAUT)
        ...
        MuscleNames = {'Triceps','Biceps','DeltAnt','DeltMed',...   % Uniformisation des noms des muscles
                   'TrapSup','GrandDent','GrandPec','Extenseurs','Fl�chisseurs'};

        Data(grpi).Filename = GrpNames{grpi};
        
        DATA = EMG_SAUT(grpi).RawData;
        
        %% pr�paration du filtrage
        Fs = EMG_SAUT(grpi).Rate; %F�quence d'�chantillonnage
        Ts = 1/Fs;
        
        [b,a] = butter(2,[5 500]/(Fs/2),'bandpass'); %Filtre passe bande, fenetre recommand�e
