...
addpath(genpath('C:\Users\Deg\Documents\MATLAB\spm1d'))
...
for n=1:length(sujet)
        ...
        PathName_EMG=[pathname,'\',sujet{n},'\LA\'];
    ...
    for j = 1:8
        ...
        nbframes = round(Fs/2); % taille de la fenêtre d'analyse
        P=2100; Q=Fs;
        
        [b,a]=butter(2,[5 500]/(Fs/2),'bandpass'); %Filtre passe bande
        ...
        for k=1:size(Grps{1,j}(1).RawData,1)
            ...
            for p=1:2
                ...
                for m = 1 : length(REPS)
                    ...
                    emg(m+length(Grps{1,j}(1).reps(2:end-1))*(p-1),:)=resample(tp(tdeb+1:tfin),P,Q);
                    emg_filt(m+length(Grps{1,j}(1).reps(2:end-1))*(p-1),:)=resample(tp_filt(tdeb+1:tfin),P,Q);
                    emg_env(m+length(Grps{1,j}(1).reps(2:end-1))*(p-1),:)=resample(yupper(tdeb+1:tfin),P,Q);
            ...
            for p=1:2
                ...
                for m=1:length(REPS)
                    ...
                    emg_ali(m+length(Grps{1,j}(1).reps(2:end-1))*(p-1),:)=resample(tp(tdeb+1+delta+D(m):tfin+delta+D(m)),P,Q);
                    emg_ali_filt(m+length(Grps{1,j}(1).reps(2:end-1))*(p-1),:)=resample(tp_filt(tdeb+1+delta+D_filt(m):tfin+delta+D_filt(m)),P,Q);
                    emg_ali_env(m+length(Grps{1,j}(1).reps(2:end-1))*(p-1),:)=resample(yupper(tdeb+1+delta+D_env(m):tfin+delta+D_env(m)),P,Q);
...
% 
            ...
            save([pathname,'\',sujet{n},'\EMG_grp_100ms.mat'],'GRP')
        end
    end
end
